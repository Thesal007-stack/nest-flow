...
export class UserService {
  ...
  public async createUserAddress(userId: string, addressDto: CreateAddressDto) {
    ...
    try {
      const address =
        await this.addressService.createAddressWithoutSave(addressDto);
  ...
  public async updateUserAddress(userId: string, addressDto: UpdateAddressDto) {
    const user = await this.getUserById(userId);
    if (!user.address) throw new NotFoundException('Address not found');
    try {
  ...
  public async getAllPrivatesFileFromAWS(userId: string) {
    try {
      const userWithFiles =
        await this.userRepository.getUserWithFilesById(userId);
      if (!userWithFiles) {
        throw new NotFoundException('User not found');
      }
      return Promise.all(
